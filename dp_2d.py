...
def shrink_2d(array2d, mask, (row_small, col_small), fillValue=-9999.):
    ...
    ary_small = masked_ary.reshape(row_small, row // row_small, col_small, col // col_small).mean(3).mean(1)
...
def smooth_2d(array2d, mask):
    m = ~mask
    r = array2d * m  # set all 'masked' points to 0. so they aren't used in the smoothing
...
def add_extra_rowcol_around_2d(ary2d, urow=1, drow=1, lcol=1, rcol=1, fillValue=np.NaN):
    ...
    row , col = ary2d.shape
    # add extra col
    ary2d = np.c_[np.full((row, lcol), fillValue), ary2d, np.full((row, rcol), fillValue) ]
...
def rolling_2d_window_pro(ary2d, window, d_i, d_j, p_i, p_j):
    ...
    strides = ary2d.strides * 2
    ary4d = np.lib.stride_tricks.as_strided(ary2d, shape=shape, strides=strides)
    # 根据新窗口大小 和 原窗口大小计算步长
